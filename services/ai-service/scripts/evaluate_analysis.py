...
def main():
    ...
    for idx, case in enumerate(cases):
        ...
        print(
            f"  Domain  -> Expected: {exp_dom:<20} | Predicted: {result.domain:<20} | [{'PASS' if domain_ok else 'FAIL'}]"
        )
        print(
            f"  Severity-> Expected: {exp_sev:<20} | Predicted: {result.severity:<20} | [{'PASS' if severity_ok else 'FAIL'}]"
        )
        print("-" * 50)
    ...
    print("\nEvaluation Summary:")
    print(
        f"Domain Accuracy   : {domain_acc:.1f}% ({correct_domain}/{len(cases)} correct)"
    )
    print(
        f"Severity Accuracy : {severity_acc:.1f}% ({correct_severity}/{len(cases)} correct)"
    )
