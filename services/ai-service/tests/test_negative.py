...
def test_does_not_assume_electricity_problem():
    ...
    assert not any("grid" in kf.lower() or "electricity" in kf.lower() for kf in validated.keyFactors)
...
def test_marks_multi_domain_problem_correctly():
    ...
    assert any(d in res["secondaryDomains"] for d in ["Disaster Management", "Urban Infrastructure", "Transportation"])
