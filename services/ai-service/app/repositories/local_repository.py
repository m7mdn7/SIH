...
class LocalRepository(BaseRepository):
    def __init__(self):
        self.challenges_file = os.path.join(settings.DATA_DIR, "challenges", "sample_challenges.json")
        ...
        if os.path.exists(self.challenges_file):
            try:
                with open(self.challenges_file, 'r', encoding='utf-8') as f:
                    ...
                    challenges_list = content if isinstance(content, list) else content.get("challenges", [])
                    for ch in challenges_list:
                        # Index by id
                        ch_id = ch.get("id") or ch.get("challengeId")
                        if ch_id:
                            self.data[ch_id] = ch
                logger.info(f"[LocalRepository] Loaded {len(self.data)} challenges from {self.challenges_file}")
            except Exception as e:
                logger.error(f"[LocalRepository] Failed to parse challenges file: {e}")
        else:
            logger.warning(f"[LocalRepository] Challenges file not found at {self.challenges_file}. Initializing empty.")
    ...
    def _save_data(self):
        try:
            ...
            logger.info(f"[LocalRepository] Saved {len(self.data)} challenges to {self.challenges_file}")
        except Exception as e:
