...
class EvidenceService:
    ...
    def extract_evidence(self, title: str, description: str, domain: str) -> dict:
        ...
        for sentence in sentences:
            ...
            match = re.search(r"\b(because|due to|since|leads to|causing|causes|resulting in|so that)\b", sentence, re.IGNORECASE)
            if match:
                has_causal = True
                ...
                if part_a and part_b:
                    if connector in ["because", "due to", "since"]:
                        inferences.append(f"{part_b.capitalize()} may be a contributing factor to {part_a.lower()}.")
                    else:
                        inferences.append(f"{part_a.capitalize()} may lead to {part_b.lower()}.")
            else:
                # Add sentence as a supported fact
                fact = sentence[0].upper() + sentence[1:]
                if not fact.endswith("."):
                    fact += "."
                if any(w in fact.lower() for w in ["might", "may", "could", "possibly", "hypothesis", "assume", "perhaps"]):
        ...
        if not hypotheses:
            if domain == "Agriculture":
                hypotheses.append("Existing storage solutions may be financially inaccessible.")
            elif domain == "Water Management":
                hypotheses.append("Contamination may originate from geological formations or nearby agricultural runoff.")
            elif domain == "Education":
                hypotheses.append("Lack of resources might be causing dropouts.")
            else:
                hypotheses.append("Local constraints might be hindering infrastructure development.")
