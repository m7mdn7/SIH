...
class ClassificationService:
    def __init__(self):
        ...
        try:
            ...
            logger.error(f"[ClassificationService] Failed to load embedding service: {e}")
    ...
    def load_taxonomy(self):
        ...
        if os.path.exists(taxonomy_path):
            ...
            logger.warning(f"[ClassificationService] Taxonomy file not found at {taxonomy_path}")
    ...
    def load_classifier(self):
        ...
        if os.path.exists(clf_path) and os.path.exists(le_path):
            try:
                ...
                logger.info("[ClassificationService] Supervised classifier loaded successfully.")
            except Exception as e:
                logger.error(f"[ClassificationService] Failed to load joblib classifier: {e}")
        else:
            logger.warning("[ClassificationService] Classifier joblib files missing. Falling back to semantic/rule classifier.")
    ...
    def init_prototypes(self):
        ...
        try:
            logger.info("[ClassificationService] Pre-encoding domain prototype embeddings...")
            ...
            for dom in self.taxonomy["domains"]:
                ...
                self.domain_prototypes[name] = self.embedding_service.encode(prototype_text)
            logger.info("[ClassificationService] Domain prototypes initialized.")
        except Exception as e:
            logger.error(f"[ClassificationService] Failed to initialize domain prototypes: {e}")
    ...
    def calculate_taxonomy_scores(self, text: str) -> dict[str, float]:
        ...
        if not self.taxonomy:
            return {d: 0.0 for d in ["Agriculture", "Water Management", "Healthcare", "Education", "Other"]}
        ...
        for dom in self.taxonomy["domains"]:
            ...
            score = len(overlap) / (math_log := np.log(len(words) + 1.0) + 1.0)
        ...
        try:
            ...
            for name, proto in self.domain_prototypes.items():
                ...
                if norm_emb > 0 and norm_proto > 0:
                    sim = float(np.dot(embedding_arr, proto_arr) / (norm_emb * norm_proto))
    ...
    def classify(self, title: str, description: str) -> dict:
        ...
        is_rejected, status_fallback = self.run_safety_validation(norm_title, norm_description)
        if is_rejected:
        ...
        if self.classifier and self.label_encoder and self.embedding_service:
            try:
                ...
                logger.error(f"[ClassificationService] Supervised prediction failed: {e}")
        ...
        final_scores = {}
        all_classes = self.label_encoder.classes_.tolist() if self.label_encoder else list(tax_scores.keys())
        ...
        # Sort domains
        sorted_domains = sorted(final_scores.items(), key=lambda item: item[1], reverse=True)
        ...
        if status in ["insufficient_information", "unknown"] and primary_domain != "Other":
