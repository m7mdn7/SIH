...
from app.taxonomy.domains import DOMAINS, DOMAIN_KEYWORDS
...
class ChallengeAnalyzer:
    ...
    def get_deterministic_domain(self, title: str, description: str) -> Optional[str]:
        ...
        for domain, keywords in DOMAIN_KEYWORDS.items():
            matches = sum(1 for keyword in keywords if f" {keyword} " in f" {combined_text} ")
    ...
    def analyze(self, challenge_id: str, title: str, description: str) -> ChallengeAIAnalysis:
        ...
        if provider_name == "openai" and self.openai_provider.client:
            try:
                analysis = self.openai_provider.analyze_challenge(challenge_id, title, description)
            except Exception as e:
                logger.warning(f"OpenAI analysis failed, falling back to mock provider. Error: {e}")
                provider_name = "mock"

        if not analysis:
            # Run mock provider
            analysis = self.mock_provider.analyze_challenge(challenge_id, title, description)

        # Taxonomy Validation
        if analysis.domain not in DOMAINS:
            logger.warning(f"Classified domain '{analysis.domain}' is not in controlled taxonomy. Running fallback.")
