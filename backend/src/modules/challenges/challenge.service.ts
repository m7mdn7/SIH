...
export class ChallengeService {
  ...
  async listChallenges(query: ChallengeQueryInput) {
    try {
      ...
      // Fallback to dev dataset below
    }

    // 5 PostGIS Locations across Jharkhand mapped by severity
    const allFallback = [
      {
        id: 'CHL-1028',
  ...
  async getGeoJSONFeatures() {
    ...
    // 5 PostGIS Locations across Jharkhand mapped by severity (Critical=Red, High=Orange, Medium=Yellow, Low=Green)
    return {
      type: 'FeatureCollection',
      features: [
        {
