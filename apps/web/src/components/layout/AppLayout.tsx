...
import { Shield, Home, AlertTriangle, Briefcase, Map, Settings, LogOut, FileText } from 'lucide-react';
...
export function AppLayout() {
  ...
  // If no user, ideally ProtectedRoute handles it, but just in case
  if (!user) return null;
    ...
    <div className="flex h-screen bg-gov-slate-50 overflow-hidden font-sans">
      ...
      <aside className="w-56 bg-gov-slate-900 text-white flex flex-col transition-all duration-300 border-r-2 border-gov-slate-800">
        ...
        <div className="p-2 border-t border-gov-slate-700 bg-gov-slate-900 space-y-0.5">
          ...
          <button onClick={handleLogout} className="flex items-center w-full px-3 py-2 text-xs font-bold uppercase tracking-wider rounded-none transition-colors text-red-500 hover:bg-red-950 hover:text-red-400 border-l-2 border-transparent">
        ...
        <header className="h-14 bg-white border-b border-gov-slate-300 flex items-center justify-between px-4 z-10 shadow-sm">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-[10px] text-gov-slate-500 uppercase tracking-widest font-bold">Social Innovation & Integration Platform</p>
            </div>
          </div>
